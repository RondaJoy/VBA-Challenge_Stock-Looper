Attribute VB_Name = "MasterSub_2"
Option Explicit

Sub MasterSub():

    Call ProcessWorksheets
...
Dim ws As Worksheet
    For Each ws In ThisWorkbook.Worksheets
        ws.Activate
        Call TickerTotal(ws)
        Call YearlyChange(ws)
        Call CellColor(ws)
        Call TotalVol(ws)
        Call Calculations(ws)
    Next ws

End Sub

Sub TickerTotal(ws As Worksheet):
...
Sub YearlyChange(ws As Worksheet):
...
Sub Calculations(ws As Worksheet):
...
' Set target range for summary data
Set k = ws.Range("K:K")
Set l = ws.Range("L:L")

' Find max value (% increase)
MX = Application.WorksheetFunction.Max(k)
ws.Range("P2").Value = Format(MX, "0.00%")

' Find the row number where the maximum value exists in column K
MXrow = Application.WorksheetFunction.Match(MX, k, 0)

' Set the reference cell based on the row and column
Set MXcell = ws.Cells(MXrow, 11)

' Use Offset to get the ticker name from the reference cell
Dim MXticker As String
MXticker = MXcell.Offset(0, -2).Value
ws.Range("O2").Value = MXticker

' Find min value (% decrease)
MN = Application.WorksheetFunction.Min(k)
ws.Range("P3").Value = Format(MN, "0.00%")

' Find the row number where the maximum value exists in column K
MNrow = Application.WorksheetFunction.Match(MN, k, 0)

' Set the reference cell based on the row and column
Set MNcell = ws.Cells(MNrow, 11)

' Use Offset to get the ticker name from the reference cell
Dim MNticker As String
MNticker = MNcell.Offset(0, -2).Value
ws.Range("O3").Value = MNticker

' Find max total volume
TV = Application.WorksheetFunction.Max(l)
ws.Range("P4").Value = Format(CDbl(TV), "#,##0")

' Find the row number where the maximum value exists in column L
TVrow = Application.WorksheetFunction.Match(TV, l, 0)

' Set the reference cell based on the row and column
Set TVcell = ws.Cells(TVrow, 12)

' Use Offset to get the ticker name from the reference cell
Dim TVticker As String
TVticker = TVcell.Offset(0, -3).Value
ws.Range("O4").Value = TVticker
